...
function validateTrelloPipelineDir() {
  ...
  for (const file of [
    ...
    "queue-worker-core.mjs",
    "queue-worker-core.test.mjs",
    "queue_worker.test.mjs",
    "pipeline_helpers.mjs",
    "handle_reschedule_run.test.mjs",
    "smoke.test.mjs",
...
function optionalToolChecks() {
  ...
  if (caddy.status === 0) {
    const result = spawnSync(
      "caddy",
      ["validate", "--config", "Caddyfile.droplet", "--adapter", "caddyfile"],
      {
        cwd: repoRoot,
        encoding: "utf8",
        env: {
          ...
          BOARD_BASICAUTH_HASH:
            process.env.BOARD_BASICAUTH_HASH
