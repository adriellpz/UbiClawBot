...
mkdir -p "${OPENCLAW_ROOT}/data/gog-canary-bridge"
...
cp "${OPENCLAW_ROOT}/.deploy-tmp-shared/shared/"* "${OPENCLAW_ROOT}/shared/"
...
if find "${OPENCLAW_ROOT}/config/live" ! -user "$(id -u)" -print -quit 2>/dev/null | grep -q .; then
  echo "deploy failed: ${OPENCLAW_ROOT}/config/live has foreign ownership — run as root: chown -R deploy:deploy ${OPENCLAW_ROOT}/config/live" >&2
  exit 1
fi
...
sudo_deploy install -D -o root -g root -m 755 "${OPENCLAW_ROOT}/.deploy-tmp-vault-reindex-cron/deploy/host-cron/vault-reindex.sh" /root/openclaw/bin/vault-reindex.sh
...
sudo_deploy stat /root/openclaw/bin/vault-reindex.sh > /dev/null || { echo "smoke failed: missing file /root/openclaw/bin/vault-reindex.sh" >&2; exit 1; }
