...
test("repo-owned trello bridge skips Ubi wake for email hook cards created in Backlog", async (t) => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "trello-pipeline-email-hook-skip-"));
  const stateDir = path.join(tempDir, "state");
  const hookConfigDir = path.join(tempDir, "openclaw");
  fs.mkdirSync(stateDir, { recursive: true });
  fs.mkdirSync(hookConfigDir, { recursive: true });
  ...
  const response = await fetch(`http://127.0.0.1:${port}/trello?token=bridge-test`, {
    ...
    body: JSON.stringify({
      action: {
        id: "action-email-backlog-create",
        type: "createCard",
        memberCreator: { username: "systemworker" },
        data: {
          card: {
            id: "card-email-backlog-1",
            name: "P2 - Email: Schedule dentist",
            shortLink: "email-backlog-1",
          },
          list: { name: "Backlog" },
          text: "P2 - Email: Schedule dentist",
        },
      },
    }),
  });
