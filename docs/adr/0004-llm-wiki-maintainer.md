...
**Considered options:** (1) keep filing-clerk move/merge only; (2) defer **wiki/sources/** and qmd to v2; (3) full maintainer with combined **wiki log**, local **wiki search**, and contradiction workflow. We chose (3) for compounding knowledge while preserving ADR 0003’s inbox split — producers still publish synthesized pages to **raw input** only.
...
- **Producer exception — `wiki/sources/`:** producer crons (Ubi, Marcos, Cheryl) may write verbatim external source captures directly to `wiki/sources/{agent}-{YYYY-MM-DD}-{slug}.md`. These are immutable ingest-time assets, not curated wiki content. All synthesized pages still go to `raw-input/` per ADR 0003. No other direct writes to `wiki/` by producers are permitted.
